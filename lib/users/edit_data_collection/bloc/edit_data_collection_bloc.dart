...
class EditDataCollectionBloc extends Bloc<EditDataCollectionEvent, EditDataCollectionState> {
    EditRepoImpl impl;
  EditDataCollectionBloc(this.impl) : super(EditInitailState()) {
    ...
    on<UploadDataEvent>((event, emit) async{
      ...
      }else if(_selectedTemplate != null && _selectedTemplate.isNotEmpty &&_selectedTemplate == 'workplan' && workplanTName != null && workplanTName.isNotEmpty){
             final response = await impl.upload(url: TemplateRoutes.uploadTemplateData, templateName: workplanTName,
       data: event.data, token: token);
