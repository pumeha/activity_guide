...
class AuthCubit extends Cubit<AuthCubitState> {
    ...
    if (status == HttpStatus.ok) {
      ...
      if (message == 'verification') {
        await MysharedPreference()
            .setPreferences(LoginKeys.token, loginValues.token!);
        ...
        String userJson = jsonEncode(
            usersList.where((user) => user['role'] == 'user').toList());
        String subadminJson = jsonEncode(
            usersList.where((user) => user['role'] == 'sub-admin').toList());
        await Future.wait([
          MysharedPreference()
              .setPreferences(LoginKeys.role, loginValues.role!),
          MysharedPreference()
              .setPreferences(LoginKeys.token, loginValues.token!),
          MysharedPreference().setPreferences(usersLists, userJson),
          MysharedPreference().setPreferences(subAdminLists, subadminJson),
          MysharedPreference()
              .setPreferences(fullnameKey, data.data![0]['fullname'])
        ]);
        ...
        await Future.wait([
          MysharedPreference()
              .setPreferences(DashboardKey.link, loginValues.dashboardurl!),
          MysharedPreference()
              .setPreferences(LoginKeys.role, loginValues.role!),
          MysharedPreference()
              .setPreferences(LoginKeys.token, loginValues.token!),
        ...
        await MysharedPreference()
            .setPreferences(templateListKeys, templatesJson);
          ...
          if (monthlyTempleJson.isNotEmpty) {
            ...
            await Future.wait([
              MysharedPreference().setPreferences(
                  monthlyTemplateName, data.data![0][monthlyTemplateName]),
              MysharedPreference()
        ...
        if (data.data![0][workplanTemplateKey] != null) {
          final workplanTemplateJson =
              data.data![0][workplanTemplateKey] as List<dynamic>;

          if (workplanTemplateJson.isNotEmpty) {
            workplanTemplateJsonString =
                jsonEncode(workplanTemplateJson.toList());

            await Future.wait([
              MysharedPreference().setPreferences(
                  workplanTemplateKey, workplanTemplateJsonString),
              MysharedPreference().setPreferences(
                  workplanTemplateName, data.data![0][workplanTemplateName])
            ]);
          }
        }
        String additionalTemplateString;
        if (data.data![0][additionalTemplateList] != null) {
          final additionalTemplateJson =
              data.data![0][additionalTemplateList] as List<dynamic>;
          if (additionalTemplateJson.isNotEmpty) {
            additionalTemplateString =
                jsonEncode(additionalTemplateJson.toList());

            await MysharedPreference().setPreferencesWithoutEncrpytion(
                additionalTemplateList, additionalTemplateString);
          }
        }
  ...
  Future<void> resetPassword(String inputCode, String newPassword) async {
    ...
    Map<String, dynamic> response =
        await authRepositoryImpl.resetPassword(inputCode, newPassword, token!);
    GeneralJsonDart data = GeneralJsonDart.fromJson(response);
  ...
  Future<void> userVerification(
      {String? inputCode, String? newPassword}) async {
    emit(AuthLoading());
    ...
    String? token = await MysharedPreference().getPreferences(LoginKeys.token);
    Map<String, dynamic> response = await authRepositoryImpl.userVerification(
        inputCode!, newPassword!, token!);
    GeneralJsonDart data = GeneralJsonDart.fromJson(response);
  ...
  Future<void> requestTokenAgain() async {
    ...
    String? token = await MysharedPreference().getPreferences(LoginKeys.token);
    Map<String, dynamic> response =
        await authRepositoryImpl.requestTokenAgain(token!);
    GeneralJsonDart data = GeneralJsonDart.fromJson(response);
